...
class Edrone():
    ...
    def __init__(self):
        ...
        with open('cell_coords.json') as f:
            self.waypoints = json.load(f)
        self.curr_row = 1
        self.curr_col = 'A'
        self.setpoint = self.waypoints['A1']
        ...
        self.Kp = [200*0.06, 200*0.06, 500*0.06]
        self.Ki = [2*0.003, 2*0.003, 10*0.003]
        self.Kd = [4500*0.3, 4500*0.3, 4700*0.3]

        # -----------------------Add other required variables for pid here ----------------------------------------------
        self.prev_error = [0, 0, 0]
        self.max_values = [1700, 1700, 1900]
        self.min_values = [1000, 1000, 1000]
        self.mid_values = [1500, 1500, 1500]
        self.error = [0, 0, 0]
        self.d_error = [0, 0, 0]
        self.i_error = [0, 0, 0]
        self.out_pitch = 0
        ...
        self.roll_err_pub = rospy.Publisher(
            '/roll_error', Float64, queue_size=1)
        self.pitch_err_pub = rospy.Publisher(
    ...
    def pid(self):
        ...
        for i in range(3):
            ...
            self.i_error[i] += self.error[i]
        self.out_pitch = (
            self.Kp[1]*self.error[1]) + (self.Kd[1]*self.d_error[1]) + (self.Ki[1]*self.i_error[1])
        self.out_roll = (
            self.Kp[0]*self.error[0]) + (self.Kd[0]*self.d_error[0]) + (self.Ki[0]*self.i_error[0])
        self.out_throttle = (
            self.Kp[2]*self.error[2]) + (self.Kd[2]*self.d_error[2]) + (self.Ki[2]*self.i_error[2])
        self.cmd.rcPitch = self.mid_values[1]+self.out_pitch
        self.cmd.rcRoll = self.mid_values[0]-self.out_roll
        self.cmd.rcThrottle = self.mid_values[2]+self.out_throttle
        ...
        if (self.error[0] <= 0.5) and (self.error[1] <= 0.5) and (self.error[2] <= 1):
            self.timer += self.sample_time
            print('Time at '+str(self.curr_col) +
                  str(self.curr_row)+': '+str(self.timer), end='\r')
            sys.stdout.flush()
            if self.curr_row == 6 and self.curr_col == 'F' and self.timer >= self.hover_time:
                self.disarm()
                self.timer = 0
            elif self.curr_row == 6 and self.timer >= self.hover_time:
                self.curr_col = chr(ord(self.curr_col)+1)
                self.curr_row = 1
                self.setpoint = self.waypoints[str(
                    self.curr_col)+str(self.curr_row)]
                self.timer = 0
            elif self.timer >= self.hover_time:
                self.curr_row += 1
                self.setpoint = self.waypoints[str(
                    self.curr_col)+str(self.curr_row)]
                self.timer = 0

        self.pitch_err_pub.publish(self.error[1])
        self.roll_err_pub.publish(self.error[0])
